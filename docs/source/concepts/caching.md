...
Caching is disabled by default for each ``pipefunc.PipeFunc`` but can be enabled using the `cache=True` parameter of the `@pipefunc` decorator or the ``pipefunc.PipeFunc`` constructor.
When caching is enabled, the result of the function is stored in the pipeline's cache after the first call.
The actual cache lives in the ``pipefunc.Pipeline`` object and is shared across all functions in the pipeline.
It's configured at the ``pipefunc.Pipeline`` level using the `cache_type` and `cache_kwargs` parameters.
When you create a ``pipefunc.Pipeline``, you can specify the type of cache to use:
...
Individual functions have caching enabled or disabled using the `cache` parameter of the `@pipefunc` decorator or the ``pipefunc.PipeFunc`` constructor.
...
- **`lru` (Least Recently Used)**:
  ...
  This creates a ``pipefunc.cache.LRUCache`` instance.
...
- **`hybrid`**:
  ...
  Use the `shared=True` option in `cache_kwargs` when using `pipeline.map` with `parallel=True`.
  This creates a ``pipefunc.cache.HybridCache`` instance.
...
- **`disk`**:
  ...
  This creates a ``pipefunc.cache.DiskCache`` instance.
...
- **`simple`**:
  ...
  This creates a ``pipefunc.cache.SimpleCache`` instance.

You can specify the cache type using the `cache_type` parameter when creating a ``pipefunc.Pipeline``.

## Cache Keys

The cache key is computed based on the input values of each ``pipefunc.PipeFunc``.
...
When using `pipeline.map`, the cache key is computed based on the input values of each ``pipefunc.PipeFunc``. That means that _*all*_ arguments to each cached function must be "hashable" (see [section](#handling-unhashable-objects) below) for caching to work.
...
If that also fails, it will raise an `pipefunc.cache.UnhashableError`.
...
Finally, if that fails, it will raise an `pipefunc.cache.UnhashableError`.
...
When the function is called through the ``pipefunc.PipeFunc`` instance, `pipefunc` will automatically call `__pipefunc_hash__` to get the state representation and include it in the cache key.
...
- The `__pipefunc_hash__` method is only relevant when caching is enabled for the ``pipefunc.PipeFunc`` instance (i.e., `cache=True`).
...
The `pipefunc.cache` module also provides a `pipefunc.cache.memoize` decorator for general-purpose function memoization, independent of ``pipefunc.PipeFunc`` and ``pipefunc.Pipeline``.
...
- `unhashable_action`: Determines the behavior when encountering unhashable objects:
  - `"error"`: Raise an ``pipefunc.cache.UnhashableError`` (default).
...
Use the `@memoize` decorator to easily add caching to any function, even outside the context of ``pipefunc.PipeFunc`` and ``pipefunc.Pipeline``.
