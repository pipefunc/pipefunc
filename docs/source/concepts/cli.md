...
- **Configure mapping options** (e.g., parallel execution, storage method, and reuse behavior) via dedicated command‐line flags.
...
In CLI and JSON modes, additional mapping options (prefixed with `--map-`) allow you to control how the pipeline executes, including settings like the run folder, parallel execution, storage backend, and reuse behavior.
...
3. **Mapping Options:**
   Mapping-related options (e.g., `--map-run_folder`, `--map-parallel`, `--map-storage`, and `--map-reuse`) are added to the `cli` and `json` subcommands, letting you configure pipeline execution without modifying code.
